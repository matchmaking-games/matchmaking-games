...
function CargoTimelineItem({
  ...
  return <div className="relative pb-6 last:pb-0 pl-6">
      {/* Dot da timeline */}
      ...
      <div className="space-y-1">
        ...
        {cargo.descricao && <p className="text-sm text-muted-foreground line-clamp-2 whitespace-pre-line">{cargo.descricao}</p>}
        {cargo.habilidades_usadas && cargo.habilidades_usadas.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
            {cargo.habilidades_usadas.map(skill => <Badge key={skill} variant="secondary" className="text-xs">
                {skill}
              </Badge>)}
          </div>}
      </div>
...
export function ExperienceCard({
  ...
  const ActionsDropdown = () => <DropdownMenu>
  ...
  if (hasCargos) {
    return <Card className="group transition-all hover:border-primary/30">
        <CardContent className="pt-6">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex gap-4">
              ...
              <div>
                ...
                {(experience.localizacao || experience.remoto) && <p className="text-sm text-muted-foreground flex items-center gap-1">
          ...
          <div className="relative ml-5 border-l-2 border-border overflow-visible">
            {experience.cargos.map(cargo => <CargoTimelineItem key={cargo.id} cargo={cargo} />)}
          </div>
        </CardContent>
      </Card>;
  }
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex gap-4 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              ...
              {/* Location */}
              {(experience.localizacao || experience.remoto) && <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {/* Description */}
              {experience.descricao && <p className="text-sm text-muted-foreground mt-3 whitespace-pre-line">{experience.descricao}</p>}

              {/* Skills used */}
              {experience.habilidades_usadas && experience.habilidades_usadas.length > 0 && <div className="flex flex-wrap gap-2 mt-3">
                  {experience.habilidades_usadas.map(skill => <Badge key={skill} variant="secondary" className="text-xs">
                      {skill}
                    </Badge>)}
                </div>}
            </div>
