...
function StatusBadge({ status }: { status: string | null }) {
  if (status === "completed") {
    return (
      <Badge className="bg-emerald-500/15 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/15">Pago</Badge>
    );
  }
  if (status === "pending") {
    return (
      <Badge className="bg-yellow-500/15 text-yellow-400 border-yellow-500/20 hover:bg-yellow-500/15">Aguardando</Badge>
    );
  }
...
export default function Billing() {
  ...
  const { pagamentos, isLoading, error, refetch } = usePagamentos(activeStudio?.estudio.id ?? null);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-[15px]">Histórico de pagamentos do estúdio</p>
      </div>
      ...
      {!isLoading && !error && pagamentos.length > 0 && (
        <div className="space-y-3">
          {pagamentos.map((p) => (
            <div key={p.id} className="rounded-lg border border-border bg-card p-4 transition-colors hover:bg-card/80">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1 min-w-0 space-y-1">
                  <p className="font-sans font-medium text-[15px] text-foreground truncate">
                    {p.vaga?.titulo ?? <span className="text-muted-foreground">Vaga removida</span>}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {p.criado_em ? format(new Date(p.criado_em), "dd/MM/yyyy", { locale: ptBR }) : "—"}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {p.invoice_url || p.invoice_pdf_url ? (
                    ...
                    <span className="text-xs text-muted-foreground">Fatura não disponível para este pagamento.</span>
                  )}
                </div>
              </div>
            </div>
